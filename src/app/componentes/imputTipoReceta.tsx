...
import { operTipoReceta } from "../../databse/operTipoRecetaDB";
...
export const ImputTipoReceta: React.FC<DropdownProps> = ({ placeholder, onSelect }) => {
    ...
    const [tReceta, setTreceta] = useState<TipoReceta[]>([]);

    const { showTipoReceta } = operTipoReceta();
    useEffect(() => {
        // obtiene el map de la tabla prodicto -nombre, id-
        const fetchTipo = async () => {
            try {
                const result = await showTipoReceta();
                setTreceta(result);
            } catch (error) {
